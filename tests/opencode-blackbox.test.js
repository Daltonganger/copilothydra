...
async function importFresh(relativePath) {
	return await import(
		`${distFileUrl(relativePath)}?t=${Date.now()}-${Math.random()}`
	);
}
...
function discoverPluginExports(moduleNamespace) {
	return Object.entries(moduleNamespace)
		.filter(
			([name, value]) =>
				name.startsWith("CopilotHydra") && typeof value === "function",
		)
...
test("black-box host add-account flow persists config and survives restart into routed runtime auth", async () => {
	await withTempOpenCodeConfig(async (tempDir) => {
		...
		try {
			...
			assert.ok(
				addAccountMethod,
				"expected CopilotHydraSetup to expose an add-account method",
			);
			...
			if (process.platform === "darwin") {
				const { getCopilotCLIKeychainToken } = await importFresh(
					"dist/storage/copilot-cli-keychain.js",
				);
				assert.equal(
			...
			assert.ok(
				runtimeHooks?.auth,
				"expected a runtime account hook after simulated restart",
			);
	...
	await withTempOpenCodeConfig(async (tempDir) => {
		...
		try {
			...
			if (process.platform === "darwin") {
				...
				assert.equal(
					await getCopilotCLIKeychainToken("alice"),
					"gho_alice_token",
				);
			}

			const accountsAfterAlice = await readJson(
				path.join(tempDir, "copilot-accounts.json"),
			);
			...
			const accountsAfterBob = await readJson(
				path.join(tempDir, "copilot-accounts.json"),
			);
			...
			assert.equal(
				runtimeSlots.length,
				2,
				"expected exactly 2 non-empty runtime slots",
			);
			...
			const authHeaders = runtimeRequests
