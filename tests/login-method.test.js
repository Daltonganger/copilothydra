...
test("CopilotHydraSetup exposes a GitHub Copilot auth method for OpenCode auth login", async () => {
	...
	try {
		const { CopilotHydraSetup } = await import(
			`../dist/index.js?${Date.now()}`
		);
...
test("CopilotHydraSetup restores host-native Copilot state when no Hydra accounts remain", async () => {
	...
	try {
		...
		const managedState = await readJson(
			path.join(tempDir, "copilothydra-opencode-state.json"),
		);
...
test("login method can create a new account from OpenCode auth login inputs", async () => {
	...
	try {
		const { createCopilotLoginMethods } = await import(
			`../dist/auth/login-method.js?${Date.now()}`
		);
		...
		const { result: finished, output } = await captureStderr(() =>
			started.callback(),
		);
		...
		assert.equal(
			config.provider[accounts.accounts[0].providerId].models["gpt-5.4"].name,
			"GPT-5.4",
		);
		assert.equal(
			config.provider[accounts.accounts[0].providerId].models["gpt-5-mini"]
				.name,
			"GPT-5-mini",
		);
	} finally {
...
test("login method can re-auth an existing account without requiring new-account prompts", async () => {
	...
	test("fetchAccountUsageSnapshot recovers missing local secret from OpenCode auth.json and persists it", async () => {
		...
		try {
			const { fetchAccountUsageSnapshot } = await import(
				`../dist/auth/usage-snapshot.js?${Date.now()}`
			);
			const { findSecret } = await import(
...
test("createCopilotLoginMethods omits add-account when 8 active accounts already exist", async () => {
	...
	const labels = createCopilotLoginMethods(accounts).map(
		(method) => method.label,
	);
...
test("new-account authorize fails when another active account was added after methods were created", async () => {
	...
	try {
		...
		const { upsertAccount, loadAccounts } = await import(
			`../dist/storage/accounts.js?${Date.now()}`
		);
		const { createCopilotLoginMethods } = await import(
			`../dist/auth/login-method.js?${Date.now()}`
		...
		const methods = createCopilotLoginMethods(
			await loadAccounts(tempDir).then((file) => file.accounts),
		);
		const addAccountMethod = methods.find(
			(method) =>
				method.label === "GitHub Copilot (CopilotHydra) — Add new account",
		);
		...
		assert.equal(
			accounts.accounts.some((account) => account.githubUsername === "user9"),
			false,
		);
	} finally {
		delete process.env.OPENCODE_CONFIG_DIR;
