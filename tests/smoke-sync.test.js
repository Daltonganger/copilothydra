...
test("single-account sync writes provider config and disables built-in github-copilot while Hydra accounts exist", async () => {
	...
	try {
		...
		assert.deepEqual(config.provider[account.providerId].options, {
		...
		assert.equal(
			config.provider[account.providerId].models["gpt-5-mini"].name,
			"GPT-5-mini",
		);
		assert.equal(
...
test("single-account sync keeps documented baseline stable even when override flag is enabled", async () => {
	...
	try {
		...
		assert.equal(
			config.provider[account.providerId].models["gemini-3.1-pro-preview"].name,
