...
import { backfillProviderAuthEntries } from "../auth/opencode-auth-sync.js";
...
export async function syncAccountsToOpenCodeConfig(
	configPath?: string,
	configDir?: string,
): Promise<void> {
	...
	const { copilothydra: legacyManagedState, ...hostCompatibleConfig } =
		config as OpenCodeConfigFile & {
			copilothydra?: { managedDisabledProviders?: string[] };
		};

	const activeAccounts = accountsFile.accounts.filter(
	...
	// Ensure each active account has a matching oauth entry in OpenCode's auth.json
	// so that getAuth() returns a valid token for the current providerId.
	await backfillProviderAuthEntries(activeAccounts, configDir);
}
...
function reconcileBuiltInCopilotAvailability(
	...
	config: OpenCodeConfigFile;
	state: { managedDisabledProviders?: string[] };
} {
