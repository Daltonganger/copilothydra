 ...
 *   and legacy github-copilot-acct-*) will trigger those correctly without any
...
export function buildAuthLoader(
	...
	return async (getAuth, provider) => {
		debugAuth(
			`loader called for provider "${provider.id}" (account ${accountId})`,
		);
		...
		if (!stored || stored.type !== "oauth") {
			...
			debugAuth(
				`no stored oauth token for provider "${provider.id}", returning empty loader`,
			);
		...
		debugAuth(
			`resolved token for provider "${provider.id}" (expires: ${stored.expires})`,
		);
		...
		const loader: AuthLoader = {
			...
			fetch: async (request, init) => {
				...
				try {
					...
					const runtimeToken = await runSerializedTokenLifecycle(
						lease.accountId,
						async () => {
							const info = await getAuth();
							const synced = syncTokenStateFromStoredAuth(
								lease.accountId,
								info,
							);
							...
							return await runSingleFlightTokenRecovery(
								lease.accountId,
								async () => {
									const refreshed = await getAuth();
									const recovered = syncTokenStateFromStoredAuth(
										lease.accountId,
										refreshed,
									);
									if (!recovered) {
					...
					headers.set(
						"Authorization",
						`Bearer ${runtimeToken.githubOAuthToken}`,
					);
					...
					await maybeLogCopilotRequestDebug({
						providerId,
						requestedModelId,
						request,
						init,
					...
					if (
						mismatchText &&
						isCapabilityMismatchError({ message: mismatchText })
					) {
						const mismatch = await handlePlanMismatch(
							lease.accountId,
							requestedModelId,
						);
						throw new Error(
...
async function maybeLogCopilotRequestDebug(input: {
	providerId: string;
	...
	await appendFile(
		...
		`${JSON.stringify({
			...
			requestBody: await extractRequestBodyText(input.request, input.init),
			responseBody: await extractCapabilityMismatchText(input.response),
		})}\n`,
	);
}

async function extractRequestBodyText(
	request: Parameters<typeof globalThis.fetch>[0],
	init?: RequestInit,
): Promise<string> {
...
async function extractCapabilityMismatchText(
	response: Response,
): Promise<string> {
